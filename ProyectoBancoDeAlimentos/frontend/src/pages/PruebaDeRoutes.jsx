...
import { getAllProducts ,crearProducto, actualizarProducto } from "../api/InventarioApi";
import {GetCupones, GetALLCupones} from "../api/CuponesApi";
import{uploadProductPhotos,uploadProductPhotos2} from "../api/Usuario.Route"
...
export default function TestAuth() {
  ...
  const testUploadProductPhotos2 = async () => {
    ...
    try {
      ...
      const namedFiles = selectedFiles.map((file, index) => ({
        file,
        desiredName: `test_image_${index + 1}_${Date.now()}.${file.name.split('.').pop()}`
      }));
  ...
  const testSubirImagenesProducto = async () => {
    ...
    try {
      ...
      alert("Subida de imágenes al producto exitosa! Revisa la consola y la respuesta abajo.");
    } catch (err) {
      console.error("[SUBIR IMAGENES PRODUCTO] error:", err?.response?.data || err);
      alert(err?.response?.data?.message || "Error al subir imágenes al producto");
    } finally {
      setUploadLoading(false);
    }
  };
  ...
  const testActualizarProducto = async () => {
    ...
    try {
      ...
      alert("Producto actualizado exitosamente! Revisa la consola y la respuesta abajo.");
    } catch (err) {
  ...
  const testCrearProducto = async () => {
    if (!updateProductData.nombre.trim() || !updateProductData.precio_base || !updateProductData.id_subcategoria.trim() || !updateProductData.id_marca.trim()) {
    ...
    try {
      ...
      alert("Producto creado exitosamente! Revisa la consola y la respuesta abajo.");
    } catch (err) {
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          <button
            ...
            style={{ backgroundColor: "#4CAF50", color: "white", border: "none", padding: "8px 16px", borderRadius: 4 }}
          >
          ...
          <button
            ...
            disabled={uploadLoading || selectedFiles.length === 0}
            style={{ backgroundColor: "#2196F3", color: "white", border: "none", padding: "8px 16px", borderRadius: 4 }}
          >
            {uploadLoading ? "Subiendo..." : "Test uploadProductPhotos2"}
          </button>
          <button
            onClick={testSubirImagenesProducto}
            disabled={uploadLoading || selectedFiles.length === 0 || !productId.trim()}
            style={{ backgroundColor: "#FF9800", color: "white", border: "none", padding: "8px 16px", borderRadius: 4 }}
          >
        ...
        <div style={{ marginBottom: 16, padding: 12, border: "1px solid #ddd", borderRadius: 8 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12, marginBottom: 12 }}>
            <div>
              <label style={{ display: "block", marginBottom: 4, fontWeight: 500 }}>ID Producto:</label>
              <input
                type="text"
                value={updateProductData.id_producto}
                onChange={(e) => setUpdateProductData({...updateProductData, id_producto: e.target.value})}
                placeholder="ID del producto"
                style={{ width: "100%", padding: "6px 12px", border: "1px solid #ccc", borderRadius: 4 }}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setUpdateProductData({...updateProductData, nombre: e.target.value})}
                placeholder="Nombre del producto"
                style={{ width: "100%", padding: "6px 12px", border: "1px solid #ccc", borderRadius: 4 }}
              />
            </div>
            <div>
              <label style={{ display: "block", marginBottom: 4, fontWeight: 500 }}>Precio Base:</label>
              <input
                type="number"
                value={updateProductData.precio_base}
                onChange={(e) => setUpdateProductData({...updateProductData, precio_base: e.target.value})}
                placeholder="Precio base"
                style={{ width: "100%", padding: "6px 12px", border: "1px solid #ccc", borderRadius: 4 }}
              />
            </div>
            <div>
              <label style={{ display: "block", marginBottom: 4, fontWeight: 500 }}>ID Subcategoría:</label>
              <input
                type="text"
                value={updateProductData.id_subcategoria}
                onChange={(e) => setUpdateProductData({...updateProductData, id_subcategoria: e.target.value})}
                placeholder="ID subcategoría"
                style={{ width: "100%", padding: "6px 12px", border: "1px solid #ccc", borderRadius: 4 }}
              />
            </div>
            <div>
              <label style={{ display: "block", marginBottom: 4, fontWeight: 500 }}>ID Marca:</label>
              <input
                type="text"
                value={updateProductData.id_marca}
                onChange={(e) => setUpdateProductData({...updateProductData, id_marca: e.target.value})}
                placeholder="ID marca"
                style={{ width: "100%", padding: "6px 12px", border: "1px solid #ccc", borderRadius: 4 }}
              />
            </div>
            <div>
              <label style={{ display: "block", marginBottom: 4, fontWeight: 500 }}>Unidad Medida:</label>
              <select
                value={updateProductData.unidad_medida}
                onChange={(e) => setUpdateProductData({...updateProductData, unidad_medida: e.target.value})}
                style={{ width: "100%", padding: "6px 12px", border: "1px solid #ccc", borderRadius: 4 }}
              >
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <textarea
              ...
              onChange={(e) => setUpdateProductData({...updateProductData, descripcion: e.target.value})}
          ...
          <button
            ...
            style={{ backgroundColor: "#9C27B0", color: "white", border: "none", padding: "8px 16px", borderRadius: 4 }}
          >
        ...
        <div style={{ marginBottom: 16, padding: 12, border: "1px solid #ddd", borderRadius: 8 }}>
          ...
          <button
            ...
            disabled={uploadLoading || !updateProductData.nombre.trim() || !updateProductData.precio_base || !updateProductData.id_subcategoria.trim() || !updateProductData.id_marca.trim()}
            style={{ backgroundColor: "#9C27B0", color: "white", border: "none", padding: "8px 16px", borderRadius: 4 }}
          >
        ...
        {selectedFiles.length > 0 && (
          <p>Archivos seleccionados: {selectedFiles.map(f => f.name).join(", ")}</p>
        )}
        {productId && (
